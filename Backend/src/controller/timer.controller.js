...
const POMODORO_TABLE_ID =
  process.env.nocodb_table_pomodoro_sessions || "maea6o4q9t7ybeu";
const NOCODB_API_KEY =
  process.env.nocodb_token || "QEHwKlBURkmRZ2oTTBgoNSLJg3Yn1bSXDXc3vPzY";
...
const updateSessionRecord = async (sessionData) => {
  try {
    ...
    console.error(
      "Error updating session in NocoDB:",
      err.response?.data || err.message
    );
...
export const pauseTimer = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to pause timer" });
  }
};
...
export const resumeTimer = async (req, res) => {
  try {
    ...
    if (session.status !== "paused") {
      return res
        .status(400)
        .json({ success: false, message: "Session not paused" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to resume timer" });
  }
};
...
export const resetTimer = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to reset timer" });
  }
};
