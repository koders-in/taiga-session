...
export const getUserTasks = async (req, res) => {
  try {
    ...
    const countResponse = await axiosInstance.get(
      `/api/v2/tables/${TASKS_TABLE_ID}/count?where=(user_id,eq,${req.user.id})`
    );
...
export const getWeekWiseWork = async (req, res) => {
  try {
    ...
    const dateFilter = `(start_time,bw,${formatISO(sevenDaysAgo)},${formatISO(
      today
    )})`;
...
export const getDailyWork = async (req, res) => {
  try {
    ...
    const dateFilter = `(start_time,bw,${formatISO(today)},${formatISO(
      tomorrow
    )})`;
    const sessions = await getSessions(req.user.id, {
      where: `where=${dateFilter}`,
...
export const getMonthlyStats = async (req, res) => {
  try {
    ...
    const dateFilter = `(start_time,bw,${formatISO(
      startOfMonthDate
    )},${formatISO(endOfMonthDate)})`;
    const sessions = await getSessions(req.user.id, {
      where: `where=${dateFilter}`,
    });
